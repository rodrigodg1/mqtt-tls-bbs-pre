...
describe("reInterval", function () {
  it("should work as an usual setInterval", function () {
    return new Promise(function (resolve, reject) {
      ...
      reInterval(function () {
        if (Math.abs(new Date().getTime() - startTime - 1000) <= 10) resolve();
        else reject(new Error("Took too much (or not enough) time"));
      }, 1000);
    });
  });
  ...
  it("should be able to reschedule an Interval", function () {
    return new Promise(function (resolve, reject) {
      ...
      var interval = reInterval(function () {
        if (Math.abs(new Date().getTime() - startTime - 800) <= 10) resolve();
        else reject(new Error("Took too much (or not enough) time"));
      }, 500);
