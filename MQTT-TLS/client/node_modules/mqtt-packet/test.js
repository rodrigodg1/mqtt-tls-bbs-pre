...
function testGenerateOnly(name, object, buffer, opts) {
  test(name, (t) => {
    t.equal(
      mqtt.generate(object, opts).toString("hex"),
      buffer.toString("hex")
    );
    t.end();
  });
...
test("cacheNumbers get/set/unset", (t) => {
  t.true(
    mqtt.writeToStream.cacheNumbers,
    "initial state of cacheNumbers is enabled"
  );
...
testParseError(
  ...
  Buffer.from([16, 255, 255, 255, 255]),
  {}
);
testParseError(
  "Invalid variable byte integer",
  Buffer.from([16, 255, 255, 255, 128]),
  {}
);
testParseError(
  "Invalid variable byte integer",
  Buffer.from([16, 255, 255, 255, 255, 1]),
  {}
);
testParseError(
  "Invalid variable byte integer",
  Buffer.from([16, 255, 255, 255, 255, 127]),
  {}
);
testParseError(
  "Invalid variable byte integer",
  Buffer.from([16, 255, 255, 255, 255, 128]),
  {}
);
testParseError(
  "Invalid variable byte integer",
  Buffer.from([16, 255, 255, 255, 255, 255, 1]),
  {}
);
...
testParseGenerateDefaults(
  ...
  Buffer.from([16, 16, 0, 4, 77, 81, 84, 84, 5, 2, 0, 60, 3, 33, 0, 20, 0, 0]),
  {
    cmd: "connect",
    retain: false,
    qos: 0,
...
testParseAndGenerate(
  ...
  Buffer.from([
    32,
    2, // Fixed Header (CONNACK, Remaining Length)
    0,
...
testParseAndGenerate(
  ...
  Buffer.from([
    32,
    3, // Fixed Header (CONNACK, Remaining Length)
    0,
    140, // Variable Header (Session not present, Bad authentication method)
...
testParseOnly(
  "Version 4 CONACK in Version 5 mode",
  {
    cmd: "connack",
    retain: false,
    qos: 0,
  ...
  { protocolVersion: 5 }
); // message is in version 4 format, but this client is in version 5 mode
...
testParseOnly(
  ...
  Buffer.from([
    64,
    2, // Fixed Header (PUBACK, Remaining Length)
    0,
...
testParseAndGenerate(
  ...
  Buffer.from([
    64,
    3, // Fixed Header (PUBACK, Remaining Length)
    0,
    42,
...
testParseOnly(
  ...
  Buffer.from([
    64,
    4, // Fixed Header (PUBACK, Remaining Length)
    0,
    42,
...
testParseOnly(
  ...
  Buffer.from([
    32,
    1, // Fixed Header (CONNACK, Remaining Length)
...
testParseOnly(
  ...
  Buffer.from([
    ...
    2, // Fixed Header (CONNACK, Remaining Length)
    1,
    0, // Variable Header (Session Present: 1 => true, Connect Reason code: Success, Implied no properties)
  ]),
  { protocolVersion: 5 }
);
...
testParseAndGenerate(
  ...
  Buffer.from([
    ...
    3, // Fixed Header (CONNACK, Remaining Length)
    1,
    0, // Variable Header (Session Present: 1 => true, Connect Reason code: Success)
    0, // no properties
  ]),
  { protocolVersion: 5 }
);
...
testParseOnly(
  ...
  Buffer.from([
    224,
    0, // Fixed Header (DISCONNECT, Remaining Length), Implied Reason code: Normal Disconnection
  ]),
  { protocolVersion: 5 }
);

testParseOnly(
  "Version 5 DISCONNECT test 2",
  {
    cmd: "disconnect",
    retain: false,
    qos: 0,
  ...
  Buffer.from([
    224,
    1, // Fixed Header (DISCONNECT, Remaining Length)
...
testParseAndGenerate(
  ...
  Buffer.from([
    224,
    2, // Fixed Header (DISCONNECT, Remaining Length)
    0, // reason Code (Normal disconnection)
    0, // no properties
  ]),
  { protocolVersion: 5 }
...
testParseError("Cannot parse protocolId", Buffer.from([16, 4, 0, 6, 77, 81]));
...
testParseGenerate(
  ...
  Buffer.from([
    ...
    41,
    1, // subscriptionIdentifiersAvailable
    42,
...
testParseGenerate(
  ...
  Buffer.from([
    ...
    255,
    255,
    127, // subscriptionIdentifier (max value)
    116,
    101,
    115,
    116, // Payload (test)
...
test("split publish longer", (t) => {
  ...
  t.equal(
    parser.parse(Buffer.from(Array(payloadLength).fill(97))),
    0,
    "remaining bytes"
  );
});
...
testParseError(
  ...
  Buffer.from([
    128,
    9, // Header (subscribeqos=0length=9)
    0,
    6, // Message ID (6)
    0,
    4, // Topic length,
...
testParseGenerate(
  ...
  Buffer.from([
    130,
    9, // Header (subscribeqos=1length=9)
    0,
    6, // Message ID (6)
    0,
    4, // Topic length,
...
testParseGenerate(
  ...
  Buffer.from([
    130,
    26, // Header (subscribeqos=1length=9)
    0,
    6, // Message ID (6)
...
testParseGenerate(
  ...
  Buffer.from([
    130,
    23, // Header (publishqos=1length=9)
    0,
    6, // Message ID (6)
    0,
    4, // Topic length,
...
testParseGenerate(
  ...
  Buffer.from([
    130,
    40, // Header (subscribeqos=1length=9)
    0,
    6, // Message ID (6)
    16, // properties length
    11,
...
testParseGenerate(
  ...
  Buffer.from([
    ...
    1,
    2,
    128, // Granted qos (0, 1, 2) and a rejected being 0x80
  ])
);
...
testParseError(
  "Packet too short",
  Buffer.from([16, 9, 0, 6, 77, 81, 73, 115, 100, 112, 3])
);
...
testParseError(
  ...
  Buffer.from([
    ...
    8, // Fixed header
    0,
    15, // string length 15 --> 15 > 8 --> error!
    77,
    81,
    73,
    115,
...
testParseError(
  ...
  Buffer.from([
    130,
    14, // subscribe header and remaining length
    0,
    123, // packet ID
    0,
    10, // topic filter length
    104,
    105,
    106,
    107,
    ...
    109,
    110,
    111, // topic filter with length of 9 bytes
    0, // requested QoS
  ])
);
...
test("stops parsing after first error", (t) => {
  ...
  parser.on("packet", (packet) => {
    t.ok(
      ...
      `expected <= ${expectedPackets} packets`
    );
  });
  ...
  parser.on("error", (erroneous) => {
    t.ok(
      ...
      `expected <= ${expectedErrors} errors`
    );
  });
